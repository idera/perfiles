...
<!--
  ...
  -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gmi="http://www.isotc211.org/2005/gmi" xmlns:gn="http://www.fao.org/geonetwork" xmlns:gn-fn-iso19139="http://geonetwork-opensource.org/xsl/functions/profiles/iso19139" version="2.0" exclude-result-prefixes="#all">
  ...
  * All elements having no child text (ie. normalize-space return '') and
  ...
  -->
	<xsl:function name="gn-fn-iso19139:vacuum" as="node()">
		...
		<xsl:for-each select="$metadata/*">
			<xsl:apply-templates mode="vacuum-iso19139" select="."/>
		</xsl:for-each>
	</xsl:function>
	<xsl:function name="gn-fn-iso19139:isElementOrChildEmpty" as="xs:boolean">
	...
	<xsl:template mode="vacuum-iso19139" match="@*|node()">
		<xsl:variable name="isElementEmpty" select="gn-fn-iso19139:isElementOrChildEmpty(.)"/>
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:copy>
					<xsl:apply-templates mode="vacuum-iso19139" select="@*|node()"/>
				</xsl:copy>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
  ...
  <xsl:template mode="vacuum-iso19139"
	...
	<xsl:template mode="vacuum-iso19139" match="gn:*" priority="2"/>
</xsl:stylesheet>
