...
<!--
  ...
  -->
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:gmi="http://www.isotc211.org/2005/gmi" xmlns:gml="http://www.opengis.net/gml" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:geonet="http://www.fao.org/geonetwork" xmlns:util="java:org.fao.geonet.util.XslUtil" xmlns:skos="http://www.w3.org/2004/02/skos/core#" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" exclude-result-prefixes="#all">
	...
	<xsl:template match="/">
		<Documents>
			<xsl:for-each select="/*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/gmd:locale/gmd:PT_Locale">
				<xsl:call-template name="document">
					<xsl:with-param name="isoLangId" select="util:threeCharLangCode(normalize-space(string(gmd:languageCode/gmd:LanguageCode/@codeListValue)))"/>
			...
			<xsl:if test="/*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/gmd:locale/gmd:PT_Locale
                     and count(/*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/
				...
				<xsl:call-template name="document">
					...
					<xsl:with-param name="langId" select="util:twoCharLangCode(normalize-space(string($isoDocLangId)))"/>
				</xsl:call-template>
			</xsl:if>
		</Documents>
	</xsl:template>
	...
	<xsl:template name="document">
		...
		<Document locale="{$isoLangId}">
			...
			<xsl:variable name="title" select="/*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/gmd:identificationInfo//gmd:citation//gmd:title//gmd:LocalisedCharacterString[@locale=$poundLangId]"/>
			<!-- not tokenized title for sorting -->
			<xsl:choose>
			...
			<xsl:apply-templates mode="index" select="*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']">
				<xsl:with-param name="langId" select="$poundLangId"/>
			</xsl:apply-templates>
		</Document>
	</xsl:template>
	...
	<xsl:template match="*" mode="metadata">
		...
		<xsl:for-each select="gmd:contact/*/gmd:organisationName//gmd:LocalisedCharacterString[@locale=$langId]">
			...
			<Field name="responsibleParty" string="{concat($roleTranslation, '|metadata|', ., '|', $logo, '|', string-join($email, ','), '|', $individualName, '|', $positionName, '|', $address, '|', string-join($phone, ','))}" store="true" index="false"/>
		</xsl:for-each>
          ...
          <xsl:variable name="keywordWithNoThesaurus" select="//gmd:MD_Keywords[
            ...
            <xsl:if test="//gmd:MD_Keywords[gmd:thesaurusName]">,</xsl:if>
					...
					<xsl:for-each-group select="//gmd:MD_Keywords[
                                        ...
                                        count(gmd:keyword//gmd:LocalisedCharacterString[@locale = $langId and text() != '']) > 0]" group-by="gmd:thesaurusName/*/gmd:title/*/text()">
			...
			<xsl:for-each select="gmd:pointOfContact/gmd:CI_ResponsibleParty/gmd:organisationName//gmd:LocalisedCharacterString[@locale=$langId]">
				...
				<xsl:variable name="address" select="string-join(../../../../gmd:contactInfo/*/gmd:address/*/(
                                    gmd:deliveryPoint|gmd:postalCode|gmd:city|
                                    gmd:administrativeArea|gmd:country)/gco:CharacterString/text(), ', ')"/>
				<Field name="responsibleParty" string="{concat($roleTranslation, '|resource|', ., '|', $logo, '|',  string-join($email, ','), '|', $individualName, '|', $positionName, '|', $address, '|', string-join($phone, ','))}" store="true" index="false"/>
			</xsl:for-each>
  ...
  gmd:pointOfContact/gmd:CI_ResponsibleParty/gmd:individualLastName/gco:CharacterString">
      ...
      -->
        ...
        gmd:description/*/gmd:textGroup/gmd:LocalisedCharacterString[@locale=$langId]
					...
					<xsl:variable name="title" select="if (gmd:name/*/gmd:textGroup/gmd:LocalisedCharacterString[@locale=$langId] != '')
					...
					<xsl:variable name="desc" select="if (gmd:description/*/gmd:textGroup/gmd:LocalisedCharacterString[@locale=$langId] != '')
      ...
      gmd:identificationInfo/*[contains(@gco:isoType, 'SV_ServiceIdentification')]/srv:serviceTypeVersion/gco:CharacterString">
		...
		<Field name="any" store="false" index="true">
			<xsl:attribute name="string"><xsl:for-each select="//node()[@locale=$langId]"><xsl:value-of select="concat(normalize-space(.), ' ')"/></xsl:for-each><xsl:text> </xsl:text><xsl:for-each select="//@codeListValue"><xsl:value-of select="concat(., ' ')"/></xsl:for-each></xsl:attribute>
		</Field>
		...
		<Field name="anylight" store="false" index="true">
			<xsl:attribute name="string"><xsl:for-each select="$identification/gmd:citation/gmd:CI_Citation/gmd:title//gmd:LocalisedCharacterString[@locale=$langId]|
                ...
                $identification/gmd:descriptiveKeywords/gmd:MD_Keywords/gmd:keyword//gmd:LocalisedCharacterString[@locale=$langId]"><xsl:value-of select="concat(., ' ')"/></xsl:for-each></xsl:attribute>
		</Field>
		<!-- Index all codelist -->
