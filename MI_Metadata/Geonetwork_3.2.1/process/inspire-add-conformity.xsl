...
<!--
  ...
  -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:geonet="http://www.fao.org/geonetwork" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gmi="http://www.isotc211.org/2005/gmi" xmlns:gml="http://www.opengis.net/gml" xmlns:skos="http://www.w3.org/2004/02/skos/core#" xmlns:util="java:org.fao.geonet.util.XslUtil" version="2.0" exclude-result-prefixes="#all">
	...
	<xsl:variable name="inspire-thesaurus" select="document(concat('file:///', replace(util:getConfigValue('codeListDir'), '\\', '/'), '/external/thesauri/theme/inspire-theme.rdf'))"/>
	...
	<xsl:template name="analyze-inspire-add-conformity">
		<xsl:param name="root"/>
		<!-- TODO : PT_FreeText ? -->
		<xsl:variable name="inspire-theme-found" select="count($inspire-thesaurus//skos:Concept[skos:prefLabel = $root//gmd:keyword/gco:CharacterString])"/>
		<!-- Check no conformity -->
	...
	<xsl:template match="/gmd:MD_Metadata|/*[@gco:isoType='gmd:MD_Metadata']">
        ...
        gmd:dataQualityInfo"/>
			...
			<xsl:variable name="metadataLanguage" select="//gmd:MD_Metadata/gmd:language/gmd:LanguageCode/@codeListValue"/>
			<xsl:variable name="metadataInspireThemes" select="$inspire-thesaurus//skos:Concept[skos:prefLabel = $keywords]/
                                skos:prefLabel[@xml:lang='en']"/>
			<xsl:variable name="defaultTitles" select="$specificationTitles/eng/
                                spec[@theme = $metadataInspireThemes/text()]"/>
			<xsl:choose>
				<xsl:when test="$metadataInspireThemes">
					<xsl:for-each select="$metadataInspireThemes">
						<xsl:variable name="current" select="."/>
						<xsl:variable name="defaultTitle" select="$specificationTitles/eng/
                                spec[lower-case(@theme) = lower-case($current)]/@title"/>
						<xsl:variable name="title" select="$specificationTitles/*[name() = $metadataLanguage]/
                                spec[lower-case(@theme) = lower-case($current)]/@title"/>
						<xsl:if test="$defaultTitle">
							<xsl:variable name="date" select="$specificationTitles/eng/
                                spec[lower-case(@theme) = lower-case($current)]/@date"/>
							<xsl:call-template name="generateDataQualityReport">
								<xsl:with-param name="title" select="if (normalize-space(title) = '')
                                        then $defaultTitle
                                        else $title"/>
								<xsl:with-param name="date" select="if ($date)
                                        then $date
          ...
          $theme != 'protected sites'">
			...
			<xsl:if test="$isThemesFromAnnexIIorIII">
				...
				<xsl:call-template name="generateDataQualityReport">
					...
					<xsl:with-param name="date" select="if ($specificationTitles/eng/spec[not(@theme)]/@date)
