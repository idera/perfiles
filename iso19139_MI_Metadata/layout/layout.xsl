...
<!--
  ...
  -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gts="http://www.isotc211.org/2005/gts" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:gml="http://www.opengis.net/gml" xmlns:gmi="http://www.isotc211.org/2005/gmi" xmlns:gn="http://www.fao.org/geonetwork" xmlns:gn-fn-metadata="http://geonetwork-opensource.org/xsl/functions/metadata" xmlns:gn-fn-iso19139="http://geonetwork-opensource.org/xsl/functions/profiles/iso19139" version="2.0" exclude-result-prefixes="#all">
	...
	<xsl:template mode="mode-iso19139" match="gml:*[
  ...
  the editor mode is not flat mode. -->
	<xsl:template mode="mode-iso19139" match="gn:child" priority="2000">
      ...
      (not($isFlatMode) or $flatModeException)">
			...
			<xsl:variable name="label" select="gn-fn-metadata:getLabel($schema, $name, $labels, name(..), '', '')"/>
  ...
  -->
	<xsl:template mode="mode-iso19139" priority="200" match="*[name() = $editorConfig/editor/fieldsWithFieldset/name
	...
	<xsl:template mode="mode-iso19139" priority="200" match="*[gco:CharacterString|gco:Integer|gco:Decimal|
       ...
       gco:Scale|gco:Record|gco:RecordType|gmx:MimeFileType|gmd:URL|gco:LocalName|gmd:PT_FreeText]">
		...
		<xsl:variable name="excluded" select="(
		...
		<xsl:variable name="hasOnlyPTFreeText" select="count(gmd:PT_FreeText) > 0 and count(gco:CharacterString) = 0"/>
		<xsl:variable name="isMultilingualElement" select="$metadataIsMultilingual and $excluded = false()"/>
		<xsl:variable name="isMultilingualElementExpanded" select="$isMultilingualElement and count($editorConfig/editor/multilingualFields/expanded[name = $elementName]) > 0"/>
		...
		<xsl:variable name="values">
			<xsl:if test="$isMultilingualElement">
                ...
                gmd:LocalisedCharacterString[@locale = concat('#',$currentLanguageId)]) = 0">
							<value ref="lang_{@id}_{$theElement/parent::node()/gn:element/@ref}" lang="{@id}"/>
						</xsl:if>
					</xsl:for-each>
				</values>
			</xsl:if>
  ...
  -->
	<xsl:template mode="mode-iso19139" match="*[gn:element/gn:text]">
		...
		<xsl:call-template name="render-element">
			...
			<xsl:with-param name="name" select="gn:element/@ref"/>
			<xsl:with-param name="editInfo" select="../gn:element"/>
			<xsl:with-param name="listOfValues" select="gn-fn-metadata:getCodeListValues($schema, name(), $codelists, .)"/>
			<xsl:with-param name="xpath" select="$xpath"/>
		</xsl:call-template>
	</xsl:template>
