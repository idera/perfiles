...
<!--
  ...
  -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gmi="http://www.isotc211.org/2005/gmi" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:gml="http://www.opengis.net/gml" xmlns:gn="http://www.fao.org/geonetwork" xmlns:gn-fn-core="http://geonetwork-opensource.org/xsl/functions/core" xmlns:gn-fn-iso19139="http://geonetwork-opensource.org/xsl/functions/profiles/iso19139" exclude-result-prefixes="#all" version="2.0">
	...
	<xsl:template name="iso19139-brief">
		...
		<xsl:for-each select="gmd:distributionInfo/gmd:MD_Distribution/gmd:transferOptions/gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource">
			...
			<xsl:if test="string($linkage)!=''">
				<xsl:element name="link">
					...
					<xsl:attribute name="type" select="gn-fn-core:protocolMimeType($linkage, $protocol, $mimeType)"/>
				</xsl:element>
			</xsl:if>
			...
			<xsl:if test="string($linkage)!='' and starts-with($protocol,'OGC:WMS-') and contains($protocol,'-get-map') and string($linkage)!='' and string($name)!=''">
				<xsl:element name="link">
					...
					<xsl:attribute name="href"><xsl:value-of select="concat('http://',$env/system/server/host,':',$env/system/server/port,
              /root/gui/locService, '/google.kml?uuid=', $uuid, '&amp;layers=', $name)"/></xsl:attribute>
		...
		<gn:info>
			...
			<xsl:choose>
				<xsl:when test="$env/harvester/enableEditing='false' and gn:info/isHarvested='y' and gn:info/edit='true'">
					<edit>false</edit>
				</xsl:when>
				<xsl:otherwise>
