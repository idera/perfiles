...
<!--
  ...
  -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exslt="http://exslt.org/common" xmlns:geonet="http://www.fao.org/geonetwork" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gmi="http://www.isotc211.org/2005/gmi" xmlns:math="http://exslt.org/math" version="2.0" exclude-result-prefixes="srv gco gmd exslt geonet math">
	...
	<xsl:variable name="setDynamicGraphicOverviewMode" select="geonet:parseBoolean($setDynamicGraphicOverview)"/>
	<!-- Load the capabilities document if one oneline resource contains a protocol set to WMS
  -->
	<xsl:variable name="onlineNodes" select="//gmd:CI_OnlineResource[contains(gmd:protocol/gco:CharacterString, 'OGC:WMS') and normalize-space(gmd:linkage/gmd:URL)=$wmsServiceUrl]"/>
	...
	<xsl:template name="analyze-add-info-from-wms">
		<xsl:param name="root"/>
		<xsl:variable name="onlineResources" select="$root//gmd:onLine/gmd:CI_OnlineResource[contains(gmd:protocol/gco:CharacterString, 'OGC:WMS')
                                            and normalize-space(gmd:linkage/gmd:URL)!='']"/>
		<xsl:variable name="srv" select="$root//*[local-name(.)='SV_ServiceIdentification' or contains(@gco:isoType, 'SV_ServiceIdentification')]"/>
...
-->
		<xsl:for-each select="$onlineResources">
			<suggestion process="add-info-from-wms" id="{generate-id()}" category="onlineSrc" target="gmd:extent">
				<name>
					<xsl:value-of select="geonet:i18n($wms-info-loc, 'a', $guiLang)"/>
					<xsl:value-of select="./gmd:linkage/gmd:URL"/>
					<xsl:value-of select="geonet:i18n($wms-info-loc, 'b', $guiLang)"/>
          ...
          "setAndReplaceExtent":{"type":"boolean", "defaultValue":"<xsl:value-of select="$setAndReplaceExtent"/>"}, "setCRS":{"type":"boolean", "defaultValue":"<xsl:value-of select="$setCRS"/>"},
          <xsl:if test="not($srv)">
	...
	<xsl:template match="gmd:identificationInfo/*" priority="2">
		<xsl:variable name="srv" select="local-name(.)='SV_ServiceIdentification'
				...
				<xsl:if test="$wmsBbox/@minx!=''">
					<gmd:graphicOverview>
						<gmd:MD_BrowseGraphic>
							<gmd:fileName>
								<gco:CharacterString>
									<xsl:value-of select="geonet:get-wms-thumbnail-url($wmsServiceUrl, '1.1.1', $layerName,
      ...
      -->
			<xsl:choose>
				<xsl:when test="$setExtentMode">
					<xsl:for-each select="srv:extent|gmd:extent">
						<xsl:choose>
							<xsl:when test="gmd:EX_Extent/gmd:temporalElement or gmd:EX_Extent/gmd:verticalElement
			...
			<xsl:if test="$setExtentMode">
				<xsl:for-each select="//gmd:onLine/gmd:CI_OnlineResource[contains(gmd:protocol/gco:CharacterString, 'OGC:WMS') and gmd:linkage/gmd:URL=$wmsServiceUrl]">
            ...
            -->
			<xsl:for-each select="*[namespace-uri()!='http://www.isotc211.org/2005/gmd'
	...
	<xsl:template match="gmi:MI_Metadata|*[@gco:isoType='gmi:MI_Metadata']">
        ...
        "/>
			<xsl:for-each select="*[namespace-uri()!='http://www.isotc211.org/2005/gmd' and namespace-uri()!='http://www.fao.org/geonetwork']">
				<xsl:copy-of select="."/>
			</xsl:for-each>
		</xsl:copy>
	</xsl:template>
	...
	<xsl:template name="add-extent-for-wms">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:apply-templates select="$capabilitiesDoc//Layer[Name=$layerName]" mode="create-bbox-for-wms"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
