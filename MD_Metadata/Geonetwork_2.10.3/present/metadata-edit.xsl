...
<xsl:stylesheet version="2.0" xmlns:xsl ="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Utilice este modo en el elemento raíz para agregar campos ocultos para el editor -->
  <xsl:template mode="schema-hidden-fields" match="gmi:MI_Metadata|*[@gco:isoType='gmi:MI_Metadata']" priority="2">
    <!-- La URL del GetCapabilities -->
  ...
  <!-- Plantilla Principal - Inicio de procesamiento ISO 19139 -->
  ...
  <!-- =================================================================== -->
  <!-- por defecto: en el modo sencillo sólo una lista plana -->
  <!-- =================================================================== -->
  ...
  <xsl:template mode="iso19139" match="*|@*">
    ...
    <!-- no mostrar los elementos vacíos en el modo de vista -->
    <xsl:choose>
  ...
  <!-- ===================================================================== -->
  <!-- Los elementos que tiene que estar Recuadrados -->
  <!-- ===================================================================== -->
  ...
  <xsl:template mode="iso19139" match="gmd:identificationInfo|gmd:distributionInfo|
              ...
              *[name(..)='gmd:resourceConstraints']|gmd:spatialRepresentationInfo|gmd:pointOfContact|
              ...
              gmd:processStep|gmd:lineage|gmi:processingInformation">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
  ...
  <!-- ===================================================================== -->
  <!-- Algunos elementos gco: elements y gmx:MimeFileType se esconden -->
  <!-- ===================================================================== -->
  ...
  <!--
	OperatesOn visualizar o editar el atributo uuidref. En el modo de edición se proporciona el panel de selección
    de metadatos para establecer el uuid. En el modo de vista, se muestra el título de la metadata. 
     Nota: puede suceder que registro de metadatos vinculado no está accesible para el usuario actual. 
    En tal situación, el click en el link devolverá una excepción privilegios.
    -->
  <xsl:template mode="iso19139" match="srv:operatesOn|gmd:featureCatalogueCitation|gmd:source[name(parent::node())='gmi:LE_ProcessStep' or name(parent::node())='gmd:LI_Lineage']" priority="99">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    <xsl:variable name="text">
    ...
    <!-- referenceSystemInfo in its own box -->
      ...
      <xsl:call-template name="complexElementGuiWrapper">
        ...
        <xsl:with-param name="realname" select="gmd:distributionInfo"/>
          ...
          <!-- Definicion de Box en contentInfo (AEA) -->
    
      <xsl:call-template name="complexElementGuiWrapper">
        <xsl:with-param name="title" select="/root/gui/schemas/iso19139/labels/element[@name='gmd:contentInfo']/label"/>
        <xsl:with-param name="id" select="generate-id(/root/gui/schemas/iso19139/labels/element[@name='gmd:contentInfo']/label)"/>
        <xsl:with-param name="content">
        ...
        <xsl:with-param name="schema" select="$schema"/>
        <xsl:with-param name="group" select="/root/gui/strings/contentInfoTab"/>
        <xsl:with-param name="edit" select="$edit"/>
        <xsl:with-param name="realname"   select="'gmd:contentInfo'"/>
      </xsl:call-template>
      
          <!-- Definicion de Box en acquisitionInformation (AEA) -->
    
      <xsl:call-template name="complexElementGuiWrapper">
        <xsl:with-param name="title" select="/root/gui/schemas/iso19139/labels/element[@name='gmi:acquisitionInformation']/label"/>
        <xsl:with-param name="id" select="generate-id(/root/gui/schemas/iso19139/labels/element[@name='gmi:acquisitionInformation']/label)"/>
        <xsl:with-param name="content">
        ...
        <xsl:with-param name="schema" select="$schema"/>
        <xsl:with-param name="group" select="/root/gui/strings/acquisitionInformationTab"/>
        <xsl:with-param name="edit" select="$edit"/>
        <xsl:with-param name="realname"   select="'gmi:acquisitionInformation'"/>
      </xsl:call-template>      
...
Modificado (AEA)
    ...
    <xsl:variable name="langId">
      <xsl:call-template name="getLangId">
        ...
        <xsl:with-param name="md"
          select="ancestor-or-self::*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']" />
      </xsl:call-template>
    </xsl:variable>
  ...
  <xsl:template name="iso19139GeoPublisherButton">
    ...
    <xsl:if test="/root/gui/config/editor-geopublisher">
      ...
      <xsl:variable name="title">
        <xsl:apply-templates mode="escapeXMLEntities" select="/root/gmi:MI_Metadata/gmd:identificationInfo/*/gmd:citation/gmd:CI_Citation/gmd:title/gco:CharacterString">
          <xsl:with-param name="includingCRLF" select="true()"/>
        </xsl:apply-templates>
      </xsl:variable>
    
      <xsl:variable name="abstract">
        <xsl:apply-templates mode="escapeXMLEntities" select="/root/gmi:MI_Metadata/gmd:identificationInfo/*/gmd:abstract/gco:CharacterString"/>
      </xsl:variable>
  ...
  <!-- ===================================================================== -->
  <xsl:template mode="superBrief" match="gmi:MI_Metadata|*[@gco:isoType='gmi:MI_Metadata']" priority="2">
    <xsl:variable name="langId">
      <xsl:call-template name="getLangId">
        <xsl:with-param name="langGui" select="/root/gui/language"/>
    ...
    <xsl:if test="/root/gui/config/metadata-tab/advanced">
      <xsl:call-template name="mainTab">
        ...
        <xsl:with-param name="menu">
		  ...
		  <!-- Se incorpora la solapa de acquisitionInformation (AEA) -->
          <item label="acquisitionInformationTab">acquisitionInformation</item>          
		  <!-- ============================================================================= -->
        </xsl:with-param>
      </xsl:call-template>
    </xsl:if>    
  ...
  <xsl:template mode="get-thumbnail" match="gmi:MI_Metadata|*[@gco:isoType='gmi:MI_Metadata']">
  ...
  <xsl:template mode="get-thumbnail" match="gmd:graphicOverview">
    ...
    <xsl:variable name="info" select="ancestor::*[name(.) = 'gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/geonet:info"></xsl:variable>
  ...
  -->
  <xsl:template mode="iso19139"
    match="gmd:*[gco:CharacterString or gmd:PT_FreeText]|gmi:*[gco:CharacterString or gmd:PT_FreeText]|
