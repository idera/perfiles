...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gmi="http://www.isotc211.org/2005/gmi" xmlns:gts="http://www.isotc211.org/2005/gts" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:gml="http://www.opengis.net/gml" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:geonet="http://www.fao.org/geonetwork" xmlns:exslt="http://exslt.org/common" exclude-result-prefixes="#all">
	...
	<!-- plantilla principal - el camino hacia el procesamiento ISO19139 -->
	...
	<!-- =================================================================== -->
	<!-- Por defecto: en modo simple -->
	<!-- =================================================================== -->
	<xsl:template mode="iso19139" match="*|@*">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		<!--No nuestra los elementos vacios en el modo vista-->
		<xsl:choose>
	...
	-->
	<!--=====================================================================-->
	<xsl:template mode="iso19139" match="gmd:graphicOverview[gmd:MD_BrowseGraphic/gmd:fileDescription/gco:CharacterString='thumbnail' or gmd:MD_BrowseGraphic/gmd:fileDescription/gco:CharacterString='large_thumbnail']" priority="20"/>
	...
	<!-- ===================================================================== -->
	<!-- Elementos que deben estar recuadrados - se tiene que sumar la etiqueta en este conjunto (AEA) -->
	<!-- ===================================================================== -->
	<xsl:template mode="iso19139" match="gmd:identificationInfo|gmd:distributionInfo|gmd:descriptiveKeywords|gmd:thesaurusName|gmd:spatialRepresentationInfo|gmd:pointOfContact|gmd:dataQualityInfo|gmd:referenceSystemInfo|gmd:contentInfo|gmi:acquisitionInformation|gmd:equivalentScale|gmd:projection|gmd:ellipsoid|gmd:extent[name(..)!='gmd:EX_TemporalExtent']|gmd:geographicBox|gmd:EX_TemporalExtent|gmd:MD_Distributor|srv:containsOperations|srv:SV_CoupledResource|gmd:metadataConstraints|gmd:scope|gmd:lineage|gmi:operation|gmi:platform|gmi:softwareReference|gmi:LE_Algorithm|gmd:MD_GridSpatialRepresentation|gmd:MD_Georeferenceable|gmd:MD_Georectified|gmd:axisDimensionProperties|gmd:centerPoint|gmi:sponsor|gmi:instrument|gmd:dimension|gmd:resourceConstraints">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<!-- ===================================================================== -->
	<!-- Algunos gco: elements y  gmx:MimeFileType son escondidos (AEA) -->
	<!-- ===================================================================== -->
	<xsl:template mode="iso19139" match="gmd:*[gco:Date|gco:DateTime|gco:Integer|gco:Decimal|gco:Boolean|gco:Real|gco:Measure|gco:Length|gco:Distance|gco:Angle|gco:Scale|gco:RecordType|gmx:MimeFileType]|gmi:*[gco:Date|gco:DateTime|gco:Integer|gco:Decimal|gco:Boolean|gco:Real|gco:Measure|gco:Length|gco:Distance|gco:Angle|gco:Scale|gco:RecordType|gmx:MimeFileType]|srv:*[gco:Date|gco:DateTime|gco:Integer|gco:Decimal|gco:Boolean|gco:Real|gco:Measure|gco:Length|gco:Distance|gco:Angle|gco:Scale|gco:RecordType|gmx:MimeFileType]">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		...
		<xsl:choose>
			<xsl:when test="$edit=true()">
				<xsl:variable name="text">
					...
					<input type="text" class="md" name="_{$ref}" id="_{$ref}" onkeyup="validateNumber(this,true,true);" onchange="validateNumber(this,true,true);" value="{gco:Distance}" size="30"/>

					&#160;
					<xsl:value-of select="/root/gui/schemas/iso19139/labels/element[@name = 'uom']/label"/>
					&#160;
					<input type="text" class="md" name="_{$ref}_uom" id="_{$ref}_uom" value="{gco:Distance/@uom}" size="10"/>
		...
		<xsl:variable name="text">
			<xsl:choose>
				<xsl:when test="$edit=true()">
					...
					<input type="text" name="_{$ref}_uuidref" id="_{$ref}_uuidref" value="{./@uuidref}" size="20" onfocus="javascript:showLinkedMetadataSelectionPanel('{$ref}', 'uuidref');"/>
					<img src="../../images/find.png" alt="{/root/gui/strings/search}" title="{/root/gui/strings/search}" onclick="javascript:showLinkedMetadataSelectionPanel('{$ref}', 'uuidref');" onmouseover="this.style.cursor='pointer';"/>
				</xsl:when>
				<xsl:otherwise>
		...
		<xsl:variable name="text">
			<xsl:choose>
				<xsl:when test="$edit=true()">
					...
					<input type="text" id="Y{$ref}" class="content" value="{substring-before(substring-after($p, 'P'), 'Y')}" size="4" onchange="buildDuration('{$ref}');" onkeyup="validateNumber(this,true,false);"/>-
					<xsl:value-of select="/root/gui/strings/durationNbMonths"/>
					<input type="text" id="M{$ref}" class="content" value="{substring-before(substring-after($p, 'Y'), 'M')}" size="4" onchange="buildDuration('{$ref}');" onkeyup="validateNumber(this,true,false);"/>-
					<xsl:value-of select="/root/gui/strings/durationNbDays"/>
					<input type="text" id="D{$ref}" class="content" value="{substring-before(substring-after($p, 'M'), 'DT')}" size="4" onchange="buildDuration('{$ref}');" onkeyup="validateNumber(this,true,false);"/>
					<br/>
					<xsl:value-of select="/root/gui/strings/durationNbHours"/>
					<input type="text" id="H{$ref}" class="content" value="{substring-before(substring-after($p, 'DT'), 'H')}" size="4" onchange="buildDuration('{$ref}');" onkeyup="validateNumber(this,true,false);"/>-
					<xsl:value-of select="/root/gui/strings/durationNbMinutes"/>
					<input type="text" id="MI{$ref}" class="content" value="{substring-before(substring-after($p, 'H'), 'M')}" size="4" onchange="buildDuration('{$ref}');" onkeyup="validateNumber(this,true,false);"/>-
					<xsl:value-of select="/root/gui/strings/durationNbSeconds"/>
					<input type="text" id="S{$ref}" class="content" value="{substring-before(substring-after(substring-after($p,'M' ),'M' ), 'S')}" size="4" onchange="buildDuration('{$ref}');" onkeyup="validateNumber(this,true,true);"/>
					<br/>
	...
	<xsl:template mode="iso19139" match="gml:timeInterval">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:variable name="text">
					<xsl:choose>
						<xsl:when test="@radix and @factor">
							<xsl:value-of select=". * @factor div @radix"/>&#160;<xsl:value-of select="@unit"/>
						</xsl:when>
						<xsl:when test="@factor">
							<xsl:value-of select=". * @factor"/>&#160;<xsl:value-of select="@unit"/>
						</xsl:when>
						<xsl:when test="@radix">
							<xsl:value-of select=". div @radix"/>&#160;<xsl:value-of select="@unit"/>
						</xsl:when>
						<xsl:otherwise>
	...
	<!-- codelists -->
	<!-- ================================================================= -->
	<xsl:template mode="iso19139" match="gmd:*[*/@codeList]|gmi:*[*/@codeList]|srv:*[*/@codeList]">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
			...
			<xsl:otherwise>
				...
				<xsl:variable name="codelistCore">
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:copy-of select="/root/gui/schemas/*[name(.)='iso19139']/codelists/codelist[@name = $qname]/*"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
	...
	<xsl:template mode="iso19139" match="//gmi:MI_Metadata/gmd:characterSet|//*[@gco:isoType='gmi:MI_Metadata']/gmd:characterSet" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		<xsl:call-template name="iso19139Codelist">
			<xsl:with-param name="schema" select="$schema"/>
	...
	<!-- Metadata -->
	<!-- ==================================================================== -->
	<xsl:template mode="iso19139" match="gmi:MI_Metadata|*[@gco:isoType='gmi:MI_Metadata']">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		<xsl:param name="embedded"/>
		...
		<xsl:choose>
			<!--Opciones de visualizacion de metadatos-->
			...
			<!-- acquisitionInformation tab  (AEA) -->
			<xsl:when test="$currTab='acquisitionInformation'">
	...
	<xsl:template name="isotabs">
			...
			<xsl:call-template name="complexElementGuiWrapper">
				...
				<xsl:with-param name="content">
					<xsl:for-each select="gmd:dataQualityInfo/gmd:DQ_DataQuality">
						<xsl:apply-templates mode="elementEP" select="gmd:scope|geonet:child[string(@name)='scope']|gmd:level">
							<xsl:with-param name="schema" select="$schema"/>
							<xsl:with-param name="edit" select="$edit"/>
						</xsl:apply-templates>
						<xsl:apply-templates mode="elementEP" select="gmd:lineage|geonet:child[string(@name)='lineage']|gmd:statement">
							<xsl:with-param name="schema" select="$schema"/>
							<xsl:with-param name="edit" select="$edit"/>
						</xsl:apply-templates>
					</xsl:for-each>
		...
		<xsl:call-template name="complexElementGuiWrapper">
			<xsl:with-param name="title" select="/root/gui/schemas/iso19139/labels/element[@name='gmi:MI_Metadata']/label"/>
			<xsl:with-param name="id" select="generate-id(/root/gui/schemas/iso19139/labels/element[@name='gmi:MI_Metadata']/label)"/>
			<xsl:with-param name="content">
				<xsl:apply-templates mode="elementEP" select="gmd:language|geonet:child[string(@name)='language']">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit" select="$edit"/>
	...
	<xsl:template name="iso19139Complete">
		...
		<!-- gmi:acquisitionInformation (AEA) -->
		<xsl:apply-templates mode="elementEP" select="gmi:acquisitionInformation|geonet:child[string(@name)='acquisitionInformation']">
			<xsl:with-param name="schema" select="$schema"/>
			<xsl:with-param name="edit" select="$edit"/>
		</xsl:apply-templates>
	...
	<xsl:template name="iso19139Metadata">
		...
		<xsl:call-template name="complexElementGui">
			...
			<xsl:with-param name="edit" select="true()"/>
			<xsl:with-param name="content">
				<!-- si el padre es la raíz a continuación, mostrar no campos en las fichas -->
				<xsl:choose>
					...
					<!-- otherwise, display everything because we have embedded MI_Metadata -->
					<xsl:otherwise>
						<xsl:apply-templates mode="elementEP" select="*">
							<xsl:with-param name="schema" select="$schema"/>
							<xsl:with-param name="edit" select="$edit"/>
	...
	<!-- ============================================================================= -->
	<!--
 Se debe modificar aca el orden para poner las etiquetas como deseamos (AEA)
	...
	<xsl:template name="iso19139GetIsoLanguage" mode="iso19139GetIsoLanguage" match="*">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<!-- In view mode display other languages from gmd:locale of gmi:MI_Metadata element -->
				<xsl:if test="../gmd:locale or ../../gmd:locale">
					<xsl:text> (</xsl:text>
					<xsl:value-of select="string(/root/gui/schemas/iso19139/labels/element[@name='gmd:locale' and not(@context)]/label)"/>
					<xsl:text>:</xsl:text>
	...
	<xsl:template mode="iso19139" match="gmd:transferOptions">
		...
		<xsl:if test="$edit=false()">
			<xsl:if test="count(gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource/gmd:protocol/gco:CharacterString[contains(string(.),'download')])>1 and
							/root/*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/geonet:info/download='true'">
				<xsl:call-template name="complexElementGui">
					<xsl:with-param name="title" select="/root/gui/strings/downloadSummary"/>
					<xsl:with-param name="content">
						<tr>
							<td align="center">
								<button class="content" onclick="javascript:runFileDownloadSummary('{/root/*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/geonet:info/uuid}', '{/root/gui/strings/downloadSummary}')" type="button">
	...
	<xsl:template mode="iso19139" match="gmd:CI_OnlineResource[starts-with(gmd:protocol/gco:CharacterString,'OGC:WMS-') and contains(gmd:protocol/gco:CharacterString,'-get-map') and gmd:name]|gmd:CI_OnlineResource[gmd:protocol/gco:CharacterString = 'OGC:WMS' and string(gmd:name/gco:CharacterString)]" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		<xsl:variable name="metadata_id" select="/root/*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/geonet:info/id"/>
		<xsl:variable name="linkage" select="gmd:linkage/gmd:URL"/>
		...
		<xsl:choose>
			...
			<xsl:when test="/root/*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/geonet:info[dynamic='true'] and string($name)!='' and string($linkage)!=''">
				...
				<xsl:apply-templates mode="simpleElement" select=".">
					...
					<xsl:with-param name="text">
						<a href="{/root/gui/locService}/google.kml?uuid={/root/*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/geonet:info/uuid}&amp;layers={$name}" title="{/root/strings/interactiveMap}">
							<xsl:choose>
								<xsl:when test="string($description)!=''">
									<xsl:value-of select="$description"/>
								</xsl:when>
			...
			<xsl:when test="string(ancestor::*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/geonet:info/dynamic)='true' and string($linkage)!='' and not(string($name))">
				<xsl:apply-templates mode="simpleElement" select=".">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="title" select="/root/gui/strings/interactiveMap"/>
					<xsl:with-param name="text">
	...
	<xsl:template mode="iso19139" match="gmd:CI_OnlineResource[starts-with(gmd:protocol/gco:CharacterString,'OGC:WMS-') and contains(gmd:protocol/gco:CharacterString,'-get-capabilities')]|gmd:CI_OnlineResource[gmd:protocol/gco:CharacterString = 'OGC:WMS' and not(string(gmd:name/gco:CharacterString))]" priority="2">
		...
		<xsl:choose>
			...
			<xsl:when test="string(/root/*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/geonet:info/dynamic)='true' and string($linkage)!=''">
				<xsl:apply-templates mode="simpleElement" select=".">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="title" select="/root/gui/strings/interactiveMap"/>
					<xsl:with-param name="text">
	...
	<xsl:template mode="iso19139" match="gmd:CI_OnlineResource[starts-with(gmd:protocol/gco:CharacterString,'ESRI:AIMS-') and contains(gmd:protocol/gco:CharacterString,'-get-image') and gmd:name]" priority="2">
		...
		<xsl:choose>
			...
			<xsl:when test="string(/root/*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/geonet:info/dynamic)='true' and string($linkage)!='' and string($name)!=''">
				<xsl:apply-templates mode="simpleElement" select=".">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="title" select="/root/gui/strings/interactiveMap"/>
					<xsl:with-param name="text">
	...
	<xsl:template mode="iso19139" match="gmd:CI_OnlineResource[matches(gmd:protocol/gco:CharacterString,'^WWW:DOWNLOAD-.*-http--download.*') and gmd:name]" priority="2">
		...
		<xsl:choose>
			...
			<xsl:when test="string(ancestor::*[name(.)='gmi:MI_Metadata' or @gco:isoType='gmi:MI_Metadata']/geonet:info/download)='true' and string($linkage)!='' and not(contains($linkage,$download_check))">
				<xsl:apply-templates mode="simpleElement" select=".">
					<xsl:with-param name="schema" select="$schema"/>
				...
				<table width="100%">
					<tr>
						...
						<td align="right">
							...
							<button class="content" onclick="javascript:doFileRemoveAction('{/root/gui/locService}/resources.del','{$ref}','{$access}','{$id}')">
	...
	<xsl:template name="iso19139GeoPublisherButton">
		...
		<xsl:if test="/root/gui/config/editor-geopublisher">
			...
			<button type="button" class="content repository" onclick="javascript:showGeoPublisherPanel('{/root/*/geonet:info/id}', '{$layer}', 
				 '{$access}', 'gmd:onLine', '{ancestor::gmd:MD_DigitalTransferOptions/geonet:element/@ref}', [{$bbox}]);" alt="{/root/gui/strings/publishHelp}" title="{/root/gui/strings/geopublisherHelp}">
	...
	<xsl:template name="iso19139-brief">
		...
		<xsl:for-each select="gmd:distributionInfo/gmd:MD_Distribution/gmd:transferOptions/gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource">
			...
			<xsl:if test="string($linkage)!='' and starts-with($protocol,'OGC:WMS-') and contains($protocol,'-get-map') and string($name)!=''">
				<xsl:element name="link">
					<xsl:attribute name="title"><xsl:value-of select="$desc"/></xsl:attribute>
					<xsl:attribute name="href"><xsl:value-of select="concat(/root/gui/env/server/protocol,'://',/root/gui/env/server/host,':',/root/gui/env/server/port,/root/gui/locService,'/google.kml?uuid=',$uuid,'&amp;layers=',$name)"/></xsl:attribute>
					<xsl:attribute name="name"><xsl:value-of select="$name"/></xsl:attribute>
	...
	<xsl:template mode="briefster" match="*">
		...
		<xsl:for-each select="gmd:extent/*/gmd:temporalElement/*/gmd:extent/gml:TimePeriod">
			<temporalExtent>
				<begin>
					<xsl:apply-templates mode="brieftime" select="gml:beginPosition|gml:begin/gml:TimeInstant/gml:timePosition"/>
				</begin>
				<end>
					<xsl:apply-templates mode="brieftime" select="gml:endPosition|gml:end/gml:TimeInstant/gml:timePosition"/>
				</end>
			</temporalExtent>
		</xsl:for-each>
	...
	<xsl:template name="iso19139CompleteTab">
		...
		<xsl:if test="/root/gui/config/metadata-tab/advanced">
			...
			<!-- gmi:acquisitionInformation (AEA) -->
			<xsl:call-template name="displayTab">
				<xsl:with-param name="tab" select="'acquisitionInformation'"/>
				<xsl:with-param name="text" select="/root/gui/strings/acquisitionInformationTab"/>
				<xsl:with-param name="indent" select="'&#xA0;&#xA0;&#xA0;'"/>
				<xsl:with-param name="tabLink" select="$tabLink"/>
			</xsl:call-template>
		...
		<xsl:apply-templates mode="simpleElement" select=".">
			...
			<xsl:with-param name="text">
				<xsl:choose>
					<xsl:when test="$edit=true()">
						<xsl:variable name="lang" select="/root/gui/language"/>
						<input class="md" name="_{gco:CharacterString/geonet:element/@ref}" id="_{gco:CharacterString/geonet:element/@ref}" value="{gco:CharacterString}">
							<xsl:if test="$isXLinked">
								<xsl:attribute name="disabled">disabled</xsl:attribute>
							</xsl:if>
						</input>
						<xsl:if test="not($isXLinked)">
							<xsl:text> </xsl:text>
							<select class="md" onchange="$('_{gco:CharacterString/geonet:element/@ref}').value = this.options[this.selectedIndex].value;" size="1">
								<option name=""/>
								<xsl:for-each select="/root/gui/regions/record">
									<xsl:sort select="label/child::*[name() = $lang]" order="ascending"/>
    ...
    -->
	<xsl:template mode="iso19139" match="gmd:parentIdentifier|gmd:code[name(../..)='gmd:aggregateDataSetIdentifier']" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
		<xsl:choose>
			<xsl:when test="$edit=true()">
				<xsl:variable name="text">
					<xsl:variable name="ref" select="gco:CharacterString/geonet:element/@ref"/>
					<input onfocus="javascript:showLinkedMetadataSelectionPanel('{$ref}', '');" class="md" type="text" name="_{$ref}" id="_{$ref}" value="{gco:CharacterString/text()}" size="20"/>
					<img src="../../images/find.png" alt="{/root/gui/strings/parentSearch}" onclick="javascript:showLinkedMetadataSelectionPanel('{$ref}', '');">
						<xsl:choose>
							<xsl:when test="name()='gmd:parentIdentifier'">
								<xsl:attribute name="title"><xsl:value-of select="/root/gui/strings/parentSearch"/></xsl:attribute>
								<xsl:attribute name="alt"><xsl:value-of select="/root/gui/strings/parentSearch"/></xsl:attribute>
							</xsl:when>
							<xsl:otherwise>
								<xsl:attribute name="title"><xsl:value-of select="/root/gui/strings/siblingSearch"/></xsl:attribute>
								<xsl:attribute name="alt"><xsl:value-of select="/root/gui/strings/siblingSearch"/></xsl:attribute>
							</xsl:otherwise>
						</xsl:choose>
					</img>
				</xsl:variable>
	...
	<!--
		...
		* ISO 19139 define how to store multilingual content in a metadata
		...
		gmi:MI_Metadata/gmd:language element. All gco:CharacterString are
		then defined in that language. 
		2) In order to add translation editor
		should add a gmd:locale element in gmi:MI_Metadata:  
		<gmd:locale>
			<gmd:PT_Locale id="FR">
				<gmd:languageCode>
	...
	-->
	<xsl:template mode="iso19139" match="gmd:*[gco:CharacterString or gmd:PT_FreeText]|gmi:*[gco:CharacterString or gmd:PT_FreeText]|
	...
	<!-- =====================================================================
		...
		* the main language (gmi:MI_Metadata/gmd:language) and
	...
	<xsl:template name="localizedCharStringField">
		...
		<xsl:variable name="widget">
			<xsl:if test="$edit=true()">
				...
				<xsl:variable name="mainLang" select="string(/root/*/gmd:language/gco:CharacterString|/root/*/gmd:language/gmd:LanguageCode/@codeListValue)"/>
						...
						<td align="left">
							...
							On change, the input with selected language is displayed. Others hidden. -->
								 ...
								 is set in gmd:locale. -->
									<select class="md lang_selector" name="localization" id="localization_{geonet:element/@ref}" onchange="enableLocalInput(this);clearSuggestion('{$suggestionDiv}');" selected="true">
										<xsl:attribute name="style"><xsl:choose><xsl:when test="count($ptFreeTextTree//gmd:LocalisedCharacterString)=0">display:none;</xsl:when><xsl:otherwise>display:block;</xsl:otherwise></xsl:choose></xsl:attribute>
										<xsl:choose>
											<xsl:when test="gco:*">
												<option value="_{gco:*/geonet:element/@ref}" code="{substring-after($mainLangId, '#')}">
													<xsl:value-of select="/root/gui/isoLang/record[code=$mainLang]/label/*[name(.)=/root/gui/language]"/>
												</option>
												<xsl:for-each select="$ptFreeTextTree//gmd:LocalisedCharacterString[@locale!=$mainLangId]">
							...
							-->
									<xsl:if test="/root/gui/config/editor-google-translate = 1">
										...
										$('localization_{geonet:element/@ref}').options[$('localization_{geonet:element/@ref}').selectedIndex].readAttribute('code'));" alt="{/root/gui/strings/translateWithGoogle}" title="{/root/gui/strings/translateWithGoogle}">
											<img width="14px" src="../../images/translate.png"/>
										</a>
										<br/>
										<div id="suggestion_{gco:CharacterString/geonet:element/@ref|
									gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString[@locale=$mainLangId]/geonet:element/@ref}" style="display:none;" class="suggestion" alt="{/root/gui/strings/translateWithGoogle}" title="{/root/gui/strings/translateWithGoogle}"/>
									</xsl:if>
								</xsl:when>
							</xsl:choose>
						</td>
	...
	<xsl:template name="PT_FreeText_Tree">
		<xsl:variable name="mainLang" select="string(/root/*/gmd:language/gco:CharacterString|/root/*/gmd:language/gmd:LanguageCode/@codeListValue)"/>
		<xsl:variable name="languages" select="/root/*/gmd:locale/gmd:PT_Locale/gmd:languageCode/gmd:LanguageCode/@codeListValue"/>
		...
		<xsl:for-each select="$languages">
			...
			<xsl:variable name="language" select="/root/gui/isoLang/record[code=$code]/label/*[name(.)=$guiLang]"/>
			<gmd:PT_FreeText>
				<gmd:textGroup>
					<gmd:LocalisedCharacterString locale="{$langId}" code="{$code}" language="{$language}">
						<xsl:value-of select="$currentNode//gmd:LocalisedCharacterString[@locale=$langId]"/>
						<xsl:choose>
							<xsl:when test="$currentNode//gmd:LocalisedCharacterString[@locale=$langId]">
								<geonet:element ref="{$currentNode//gmd:LocalisedCharacterString[@locale=$langId]/geonet:element/@ref}"/>
							</xsl:when>
							<xsl:otherwise>
